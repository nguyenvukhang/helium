...
//
//		Also demonstrates various mapping tricks which may be performed
// by sending 		Apple Events to the Wacom driver. These features are
// available in the 		application's Fun menu.
//
//		When an application wishes to customize driver settings, it must
// open a 		"context" in the driver to contain its custom settings.
// That context is then 		active whenever the application is
// frontmost.
//
// COPYRIGHT
...
- (IBAction)focusCenter:(id)sender_I {
  [self makeContextForCurrentTablet];
  [self setRect:0.6 x:0.2 y:0.2];
}

- (IBAction)focusRight:(id)sender_I {
  [self makeContextForCurrentTablet];
  [self setRect:0.6 x:0.4 y:0.2];
}

- (IBAction)focusLeft:(id)sender_I {
  [self makeContextForCurrentTablet];
  [self setRect:0.6 x:0 y:0.2];
}


- (void)setRect:(float)scale x:(float)x y:(float)y {
  NSRect screen = [NSScreen screens][0].frame;
  float screenWidth = NSMaxX(screen);
  float screenHeight = NSMaxY(screen);

  float aratio = 1.6; // fixed by wacom intuous
  NSRect custom = NSZeroRect;
  custom.size.width = scale * screenWidth;
  custom.size.height = custom.size.width / aratio;
  custom.origin.x = screenWidth * x;
  custom.origin.y = screenHeight * y;
  [self setPortionOfScreen:custom];
}
...
- (IBAction)toggleConstrainToLeftHalfOfTablet:(id)sender_I {
  ...
  if (mUsingLeftHalfOfTablet == YES) {
    ...
    tabletAreaDesc = [WacomTabletDriver dataForAttribute:pMapTabletArea
                                                  ofType:typeLongRectangle
                                            routingTable:routingDesc];
    [[tabletAreaDesc data] getBytes:&mFullTabletArea
                             length:sizeof(mFullTabletArea)];
...
- (IBAction)useRealAbsoluteCoordinates:(id)sender_I {
  ...
  //			settings. Passing the desktop rect here actually serves
  //as
  // a
  // special 			flag to enable corrected absoluteXY mapping. You
  // should
  // ONLY pass 			the full desktop rect, which Cintiqs will interpret
  // specially. Passing any other value is undefined.
  [self setPortionOfScreen:[self desktopRect]];
  ...
  outputAreaDesc =
      [WacomTabletDriver dataForAttribute:pContextMapTabletOutputArea
                                   ofType:typeLongRectangle
                             routingTable:routingDesc];
  ...
  // on the tablet.
  NSLog(@"output area : l=%d, t=%d   r=%d, b=%d", outputArea.left,
        outputArea.top, outputArea.right, outputArea.bottom);
...
- (NSString *)nameOfPen:(NSUInteger)serialNumber_I {
  UInt32 transducerCount =
      [WacomTabletDriver transducerCountForTablet:(UInt32)lastUsedTablet];
  ...
  for (counter = 1; counter <= transducerCount && name == nil; ++counter) {
    // Retrieve pen info.
    // Note:	Transducer data is not available through a context. We access
    // the
    //			data directly from the tablet.
    routingTable =
        [WacomTabletDriver routingTableForTablet:(UInt32)lastUsedTablet
                                      transducer:counter];
    nameDesc = [WacomTabletDriver dataForAttribute:pName
                                            ofType:typeUTF8Text
                                      routingTable:routingTable];
    serialNumberDesc = [WacomTabletDriver dataForAttribute:pSerialNumber
                                                    ofType:typeUInt32
                                              routingTable:routingTable];
